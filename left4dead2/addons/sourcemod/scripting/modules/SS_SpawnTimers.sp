...
SpawnTimers_OnModuleStart() {
	// Timer
	hSpawnTimeMin = CreateConVar("ss_time_min", "12.0", "The minimum auto spawn time (seconds) for infected", 0, true, 0.0);
	hSpawnTimeMax = CreateConVar("ss_time_max", "15.0", "The maximum auto spawn time (seconds) for infected", 0, true, 1.0);
	hSpawnTimeMode = CreateConVar("ss_time_mode", "1", "The spawn time mode [ 0 = RANDOMIZED | 1 = INCREMENTAL | 2 = DECREMENTAL ]", 0, true, 0.0, true, 2.0);
	hCvarFrequencyBoomerAmbush = CreateConVar("ss_boomer_frequency", "7.0", "Roughly how often to attempt to ambush survivors with a boomer", 0);
