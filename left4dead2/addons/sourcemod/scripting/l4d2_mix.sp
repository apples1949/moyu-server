...
public Plugin myinfo =
{
    ...
    author = "Luckylock,modified by Bigred",
...
public Action Admin_MixStart(int client, int Args)
{
	...
	{
		CPrintToChat(client, "%t", "start_condition");
	}
	else
	{
...
public bool Menu_Initialise()
{
	...
	switch(currentState) 
	{
		...
		{
			mixMenu.SetTitle("%T", "tittle_1st_cap", LANG_SERVER);
			return true;
		}

		case STATE_SECOND_CAPT: 
		{
			mixMenu.SetTitle("%T", "tittle_2nd_cap", LANG_SERVER);
			return true;
        }

		case STATE_PICK_TEAMS: 
		{
			mixMenu.SetTitle("%T", "tittle_pteammate", LANG_SERVER);
			return true;
		}
	}
