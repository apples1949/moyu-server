...
{
	g_hSILimit = CreateConVar("ss_si_limit", "12", "Maximum number of traits that exist at the same time", _, true, 1.0, true, 32.0);
	g_hSpawnSize = CreateConVar("ss_spawn_size", "4", "How many special feelings are produced at a time", _, true, 1.0, true, 32.0);
	g_hSpawnLimits[SI_SMOKER] = CreateConVar("ss_smoker_limit",	"3", "Maximum number of smokers that exist at the same time", _, true, 0.0, true, 32.0);
	g_hSpawnLimits[SI_BOOMER] = CreateConVar("ss_boomer_limit",	"2", "Maximum number of boomers that exist at the same time", _, true, 0.0, true, 32.0);
	g_hSpawnLimits[SI_HUNTER] = CreateConVar("ss_hunter_limit",	"3", "Maximum number of hunters that exist at the same time", _, true, 0.0, true, 32.0);
	g_hSpawnLimits[SI_SPITTER] = CreateConVar("ss_spitter_limit", "2", "Maximum number of spitters that exist at the same time", _, true, 0.0, true, 32.0);
	g_hSpawnLimits[SI_JOCKEY] = CreateConVar("ss_jockey_limit",	"3", "Maximum number of jockeys that exist at the same time", _, true, 0.0, true, 32.0);
	g_hSpawnLimits[SI_CHARGER] = CreateConVar("ss_charger_limit", "3", "The maximum number of chargers that exist at the same time", _, true, 0.0, true, 32.0);

	g_hSpawnWeights[SI_SMOKER] = CreateConVar("ss_smoker_weight", "80", "smoker produces specific gravity", _, true, 0.0);
	g_hSpawnWeights[SI_BOOMER] = CreateConVar("ss_boomer_weight", "125", "boomer produces specific gravity", _, true, 0.0);
	g_hSpawnWeights[SI_HUNTER] = CreateConVar("ss_hunter_weight", "100", "hunter produces specific gravity", _, true, 0.0);
	g_hSpawnWeights[SI_SPITTER] = CreateConVar("ss_spitter_weight", "125", "spitter produces specific gravity", _, true, 0.0);
	g_hSpawnWeights[SI_JOCKEY] = CreateConVar("ss_jockey_weight", "100", "jockey produces specific gravity", _, true, 0.0);
	g_hSpawnWeights[SI_CHARGER] = CreateConVar("ss_charger_weight", "100", "charger produces specific gravity", _, true, 0.0);
	g_hScaleWeights = CreateConVar("ss_scale_weights", "1",	"[ 0 = off | 1 = on ] Scale the weight of the corresponding effect", _, true, 0.0, true, 1.0);
	g_hSpawnTimeMin = CreateConVar("ss_time_min", "10.0", "The minimum time to produce special feeling", _, true, 0.0);
	g_hSpawnTimeMax = CreateConVar("ss_time_max", "15.0", "The maximum generation time of special feeling", _, true, 1.0);
	g_hSpawnTimeMode = CreateConVar("ss_time_mode", "2", "Unique refresh time mode [ 0 = random | 1 = increment | 2 = decrement ]", _, true, 0.0, true, 2.0);

	g_hSIbase = CreateConVar("ss_base_limit", "4", "How many special feelings does the survivor team have when there are no more than 4 players", _, true, 0.0, true, 32.0);
	g_hSIextra = CreateConVar("ss_extra_limit", "1", "How many perks can be added for each additional survivor team player", _, true, 0.0, true, 32.0);
	g_hGroupbase = CreateConVar("ss_groupbase_limit", "4", "How many special senses are generated at a time when there are no more than 4 players in the survivor team", _, true, 0.0, true, 32.0);
	g_hGroupextra = CreateConVar("ss_groupextra_limit", "2", "Survivor team players will spawn an extra one for each additional player.", _, true, 1.0, true, 32.0);
	g_hRusherDistance = CreateConVar("ss_rusher_distance", "2000.0", "How much is the distance counted as running map", _, true, 500.0);
	g_hTankSpawnAction = CreateConVar("ss_tankspawn_action", "1", "Whether to modify the current spawn parameters after the tank is spawned, and restore it after the tank dies? [ 0 = ignore (keep the original spawn status) | 1 = custom ]", _, true, 0.0, true, 1.0);
	g_hTankSpawnLimits = CreateConVar("ss_tankspawn_limits", "3;1;3;0;3;3", "Custom parameters for the amount of each special effect after the tank is spawned");
	g_hTankSpawnWeights = CreateConVar("ss_tankspawn_weights", "80;300;100;0;100;100", "Custom parameters for each specific gravity after the tank is spawned");
...
void vSetMaxSpecialsCount()
{
	...
	PrintToChatAll("\x01[\x05%dspecial\x01/\x05times\x01] \x05%d special \x01[\x03%.1f\x01~\x03%.1f\x01]\x04 seconds", iTempSize, iTempLimit, g_fSpawnTimeMin, g_fSpawnTimeMax);
}
...
Action cmdType(int client, int args)
{
	if(args == 1)
	{
		...
		{
			g_iCurrentClass = UNINITIALISED;
			ReplyToCommand(client, "Single Sensation Mode turned off");
			vResetLimits();
		}
		else if(strcmp(sTargetClass, "random", false) == 0)
		{
			PrintToChatAll("\x03Current rotation\x01: \n");
			PrintToChatAll("\x01[\x05%s\x01]\x04Mode\x01", g_sZombieClass[iSetRandomType()]);
		}
		else
		{
			...
			if(iClassIndex == UNINITIALISED)
			{
				...
				ReplyToCommand(client, "The target sensibility type is the same as the current sensibility type");
			else
			{
				vSiTypeMode(iClassIndex);
				PrintToChatAll("\x01[\x05%s\x01]\x04Mode\x01", g_sZombieClass[iClassIndex]);
			}
		}
	}
	else
